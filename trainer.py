...
class TrainerBase:
    ...
    def build_model(self):
        ...
        if self.num_gpus > 1:
            self.model = DDP(model, device_ids=[self.rank,], static_graph=False, find_unused_parameters=True)  # wrap the network
        else:
            self.model = model
        ...
        if self.task == "realsr" or self.task == "rescale":
            if self.num_gpus > 1:
                self.bertmodel = DDP(BertModel.from_pretrained("google-bert/bert-base-uncased").cuda(), device_ids=[self.rank,], static_graph=False, find_unused_parameters=True)  # wrap the network
                self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            else:
                self.bertmodel = BertModel.from_pretrained("google-bert/bert-base-uncased")
                self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            print("load bert model and tokenizer")
    ...
    def save_ckpt(self):
        if self.rank == 0:
            ...
            if self.num_gpus > 1:
                self.bertmodel.module.save_pretrained(self.ckpt_dir /  'bertmodel')
            else:
                self.bertmodel.save_pretrained(self.ckpt_dir /  'bertmodel')
            self.tokenizer.save_pretrained(self.ckpt_dir / 'tokenizer')
...
class TrainerDifIR(TrainerBase):
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        elif self.task == "rescale":
            magnifyscale = data["scale"].cuda()
            prompt_txt = data["prompt"][0]
            prompt_inputs = self.tokenizer.batch_encode_plus(prompt_txt, max_length=64, padding='max_length', truncation=True, return_tensors="pt")
            prompt_outputs = self.bertmodel(**prompt_inputs)
            prompt = prompt_outputs.last_hidden_state.cuda()
        #print(prompt.shape) torch.Size([64, 37, 768])
        ...
        if realesrgan and phase == 'train':
            ...
            if self.task == "realsr":
                return {'lq':self.lq, 'gt':self.gt, 'prompt':prompt}
            elif self.task == "rescale":
                return {'lq':self.lq, 'gt':self.gt, 'scale': scale, 'prompt':prompt}
            else:
            ...
            val_r = {key:value.cuda().to(dtype=dtype) for key, value in data.items()}
            val_r["prompt"] = prompt
            if self.task == "rescale":
                val_r["scale"] = magnifyscale
            return val_r
        else:
            if self.task == "rescale":
                data["scale"] = magnifyscale
            data["prompt"] = prompt

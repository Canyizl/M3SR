...
class BaseData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        out = {'image':im_target, 'lq':im_target, 'prompt':prompt}
...
class PairedData(Dataset):
    def __init__(
        ...
        for sub_dir in os.listdir(dir_path):
            sub_pathdir = os.path.join(dir_path, sub_dir)
            all_dir_path.append(sub_pathdir)

        if dir_path is not None:
            file_paths_all.extend(util_common.scan_files_from_folder(all_dir_path, im_exts, recursive))
    ...
    def __getitem__(self, index):
        ...
        low_scale = int(subfile_name.split("_")[-2].rstrip('x'))
        high_scale = int(subfile_name.split("_")[-1].rstrip('x'))
        ...
        out = {'lq':im_lq, 'gt':im_gt, 'scale': np.array([low_scale, high_scale, round(high_scale / low_scale,1)]), 'prompt': prompt}

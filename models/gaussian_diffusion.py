...
def get_batch_lin_function(x1=4.0, y1=1.0, x2=2.0, y2=0.5):
    x1 = torch.tensor(x1) if not isinstance(x1, torch.Tensor) else x1
    y1 = torch.tensor(y1) if not isinstance(y1, torch.Tensor) else y1
    x2 = torch.tensor(x2) if not isinstance(x2, torch.Tensor) else x2
    y2 = torch.tensor(y2) if not isinstance(y2, torch.Tensor) else y2

    m = (y2 - y1) / (x2 - x1)                    
    b = y1 - m * x1                               
    ones = torch.ones_like(m)
    def batch_linear_func(x, m=m,b=b,ones=ones):
        x = torch.clamp(x, min=4, max=8)
        device = x.device
        ones = ones.unsqueeze(-1).to(device)
        m = m.unsqueeze(-1).to(device)
        b = b.unsqueeze(-1).to(device)
        return ones / (m * x + b)

    return batch_linear_func
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x_t.shape[:2]
        assert t.shape == (B,)
        model_output = model(self._scale_input(x_t, t), t, prompt=prompt, scale=scale, **model_kwargs)
    ...
    def p_sample(self, model, x, y, t, prompt=None, scale=None, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        """
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            model_output = model(self._scale_input(z_t, t), t, prompt=prompt, scale=scale, **model_kwargs)
            target = {
            ...
            if scale is None:
                ...
                terms["mse"] = mean_flat((target - model_output)** 2) * self.scale_loss_weightfn(scale[:,-1])

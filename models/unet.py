...
import cv2
from utils.util_image import rgb2ycbcr, rgb2ycbcrTorch
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, p_emb=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, BasicLayer):
                x = layer(x, p_emb)
            else:
                x = layer(x)
        return x
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb, p_emb = None):
        ...
        if p_emb == None:
            ...
            return self.skip_connection(x) + h, p_emb
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim = model_channels * 4 # 160 * 4
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim),
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        ...
        prompt_embed_dim = time_embed_dim
        self.prompt_embed = nn.Sequential(
            linear(768, prompt_embed_dim),
            nn.SiLU(),
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, padding=1))]#[TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, padding=1))]
        )
        input_block_chans = [ch]
        ds = image_size
        for level, mult in enumerate(channel_mult):
    ...
    def forward(self, x, timesteps, prompt=None, scale=None, lq=None, mask=None):
        ...
        for ii, module in enumerate(self.input_blocks):
            h = module(h, emb, prompt_emb)
            hs.append(h)
        h = self.middle_block(h, emb, prompt_emb)
...
class UNetModelConv(nn.Module):
    ...
    def __init__(
        ...
        self.film_mean =  nn.Sequential(
            linear(640, 64), #linear(model_channels, time_embed_dim),
            nn.SiLU(),
            linear(64, 1), #linear(time_embed_dim, time_embed_dim),
            nn.Sigmoid()
            )
        self.film_var =  nn.Sequential(
            linear(640, 64), #linear(model_channels, time_embed_dim),
            nn.SiLU(),
            linear(64, 1), #linear(time_embed_dim, time_embed_dim),
            nn.Sigmoid()

...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.prompt_clinear = nn.Linear(640, dim)
        self.prompt_nlinear = nn.Linear(64, 64)

    def forward(self, x, mask=None, p_emb = None):
        """
        Args:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=normalization, in_chans = None):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, p_emb = None):
        ...
        if self.input_resolution == x_size:
            attn_windows = self.attn(x_windows, mask=self.attn_mask.to(x.dtype), p_emb=p_emb)  # (NW*B) x (Ws*Ws) x C
        else:
            attn_windows = self.attn(x_windows, mask=self.calculate_mask(x_size).to(x.device, x.dtype), p_emb=p_emb)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, p_emb=None):
        ...
        if p_emb == None:
            ...
            for blk in self.blocks:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, x, p_emb)
                else:
                    x = blk(x, p_emb)
